...
import type { AgentDecl, ClientDecl, Document, ToolDecl, TypeBoxSchema, TypeDecl } from "../types.ts";
...
async function tryProviderBridge(request: LlmBridgeRequest): Promise<unknown | null> {
  ...
  if (request.client.provider === "openai" && process.env.OPENAI_API_KEY) {
    return callOpenAI(request);
  }

  if (request.client.provider === "anthropic" && process.env.ANTHROPIC_API_KEY) {
...
function mockString(
  ...
  const key = path.at(-1) ?? "";
  const company = String(kwargs.company ?? kwargs.task ?? "openclaw").toLowerCase();
  if (key.includes("url") || schema.pattern === "^https://") {
    return `https://${company.replace(/\s+/g, "-")}.com`;
  }
  if (key.includes("snippet")) {
    return `${String(kwargs.company ?? kwargs.task ?? "OpenClaw")} summary generated by the mock bridge.`;
  }
  if (key.includes("tag")) {
    return "analysis";
  }
